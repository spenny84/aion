...
public abstract class ApiAion extends Api {
    ...
    public ApiAion(final IAionChain _ac) {
        ...
        evtMgr
            .registerEvent(Collections.singletonList(new EventBlock(EventBlock.CALLBACK.ONBLOCK0)));
    }
                ...
                try {
                    ...
                    if (e.getEventType() == IHandler.TYPE.BLOCK0.getValue()
                        ...
                        if (e.getCallbackType() == EventTx.CALLBACK.PENDINGTXUPDATE0.getValue()) {
                            pendingTxUpdate((ITxReceipt) e.getFuncArgs().get(0),
                                GETSTATE((int) e.getFuncArgs().get(1)));
                        } else if (e.getCallbackType() == EventTx.CALLBACK.PENDINGTXRECEIVED0
                            .getValue()) {
    ...
    protected BlockContext getBlockTemplate() {
        ...
        try {
            AionBlock bestBlock = ((AionPendingStateImpl) ac.getAionHub().getPendingState())
                .getBestBlock();
            byte[] bestBlockHash = bestBlock.getHeader().getMineHash();

            if (currentBestBlockHash == null || !Arrays
                .equals(bestBlockHash, currentBestBlockHash)) {
    ...
    protected Map.Entry<AionBlock, BigInteger> getBlockWithTotalDifficulty(long blkNr) {
        if (blkNr > 0) {
            ...
            AionGenesis genBlk = CfgAion.inst().getGenesis();
            return Map.entry(new AionBlock(genBlk.getHeader(), genBlk.getTransactionsList()),
    ...
    protected TxRecpt getTransactionReceipt(byte[] txHash) {
        ...
        AionBlock block = this.ac.getAionHub().getBlockchain()
            .getBlockByHash(txInfo.getBlockHash());
        ...
        AionBlock mainBlock = this.ac.getAionHub().getBlockchain()
            .getBlockByNumber(block.getNumber());
    ...
    protected byte[] doCall(ArgTxCall _params) {
        AionTransaction tx = new AionTransaction(_params.getNonce().toByteArray(), _params.getTo(),
            _params.getValue().toByteArray(), _params.getData(), _params.getNrg(),
            _params.getNrgPrice());
        AionTxReceipt rec = this.ac
    ...
    protected long estimateNrg(ArgTxCall params) {
        ...
        AionTransaction tx =
            (params.getFrom() != null && !params.getFrom().equals(Address.EMPTY_ADDRESS()))
                ? new AionTransaction(params.getNonce().toByteArray(), params.getFrom(),
                    params.getTo(),
                    params.getValue().toByteArray(), params.getData(), params.getNrg(),
                    params.getNrgPrice())
                : new AionTransaction(params.getNonce().toByteArray(), params.getTo(),
                    params.getValue().toByteArray(), params.getData(), params.getNrg(),
                    params.getNrgPrice());
        ...
        if (key == null) {
            ...
            try {
                synchronized (pendingState) {
                    byte[] nonce =
                        !(_params.getNonce().equals(BigInteger.ZERO)) ? _params.getNonce()
                            .toByteArray()
                            : pendingState.bestPendingStateNonce(Address.wrap(key.getAddress()))
                                .toByteArray();

                    AionTransaction tx = new AionTransaction(nonce, from, null,
    ...
    protected byte[] sendTransaction(ArgTxCall _params) {
        ...
        try {
            synchronized (pendingState) {
                ...
                byte[] nonce =
                    (!_params.getNonce().equals(BigInteger.ZERO)) ? _params.getNonce().toByteArray()
                        : pendingState.bestPendingStateNonce(Address.wrap(key.getAddress()))
                            .toByteArray();

                AionTransaction tx = new AionTransaction(nonce, _params.getTo(),
                    _params.getValue().toByteArray(),
                    _params.getData(), _params.getNrg(), _params.getNrgPrice());
                tx.sign(key);
