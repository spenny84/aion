 ...
 *
 */
public final class ResAllBlocksHeaders extends Msg {

    private final List<A0BlockHeader> blockHeaders;

    public ResAllBlocksHeaders(final List<A0BlockHeader> _blockHeaders) {
        super(Ver.V0, Ctrl.SYNC, Act.RES_ALL_BLOCK_HEADERS);
        blockHeaders = _blockHeaders;
    }

    public static ResAllBlocksHeaders decode(final byte[] _msgBytes) {
        if (_msgBytes == null || _msgBytes.length == 0)
            ...
            try {
                ...
                return new ResAllBlocksHeaders(blockHeaders);
            } catch (Exception ex) {
                return null;
            }
        }
